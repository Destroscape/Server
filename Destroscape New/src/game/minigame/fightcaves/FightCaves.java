package game.minigame.fightcaves;
	...
	public void spawnNextWave(Player c) {
		if (c != null) {
			...
			for (int j = 0; j < npcAmount; j++) {
				...
				int atk = getAtk(npc);
				int def = getDef(npc);
				NPCHandler.spawnNpc(c, npc, X, Y, H, 0, hp, max, atk, def,
						true, false);
			}
			...
			if (c != null) {
				if (c.tzKekTimer == 0) {
					if (NPCHandler.npcs[i].isDead) {
						NPCHandler.spawnNpc(c, TZ_KEK_SPAWN, x, y,
								c.heightLevel, 0, hp, max, atk, def, true,
								false);
						NPCHandler.spawnNpc(c, TZ_KEK_SPAWN, x1, y1,
								c.heightLevel, 0, hp, max, atk, def, true,
								false);
					}
				}
