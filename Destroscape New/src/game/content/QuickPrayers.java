...
public class QuickPrayers {
	...
	public static void clickPray(Player p, int actionId) {
		canBeSelected(p, actionId);
		for (int j = 0; j < data.length; j++) {
			if (data[j][0] == actionId) {
				if (p.quickPrayers[data[j][2]]) {
					p.quickPrayers[data[j][2]] = false;
					p.getPA().sendFrame36(data[j][1], 0);
				} else {
					p.quickPrayers[data[j][2]] = true;
