...
public class PotionMixing {
	...
	private final int[][] potMixing = { { 117, 117, 113, 227 },
			...
			{ 12142, 12146, 12140, 227 } };

	public PotionMixing(final Player c) {
		this.c = c;
	}

	// use id, use id2, new id1, new id2
	public void mixPotion(final int id, final int id2) {
		for (final int[] element : potMixing) {
			if (element[0] == id && element[1] == id || element[1] == id
				...
				c.getItems().addItem(element[2], 1);
				c.getItems().addItem(element[3], 1);
				break;
			}
		}
	}
	...
	public void mixPotion2(final int id, final int id2) {
		final String id11 = c.getItems().getItemName(id);
		final String id22 = c.getItems().getItemName(id2);
		if (id11.substring(0, id11.indexOf("(")).equalsIgnoreCase(
			...
			try {
				...
				if (totalAmount > 4) {
					...
					c.getItems().deleteItem(id2, c.getItems().getItemSlot(id2),
							1);
