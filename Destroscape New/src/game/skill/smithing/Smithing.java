...
public class Smithing extends SkillHandler {
	...
	private void CheckAddy(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2361;
	...
	private void CheckBronze(final Player c, final int level,
			final int amounttomake, final String type) {
	...
	private void CheckIron(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2351;
	...
	private void CheckMith(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2359;
	...
	private void CheckRune(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2363;
	...
	private void CheckSteel(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2353;
	...
	public void doAction(final Player c, final int toadd, final int toremove,
			final int toremove2, final int timestomake, final int NOTUSED,
		...
		if (c.isSkilling[c.playerSmithing] == true) {
			return;
		}
		c.isSkilling[c.playerSmithing] = true;
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
				if (c.isSkilling[c.playerSmithing] == false) {
					container.stop();
					return;
				}
				if (!c.getItems().playerHasItem(toremove, toremove2)) {
				...
				if (c.playerEquipment[c.playerHands] == 13855) {
					c.getPA().addSkillXP(xp * SMITHING_XP * 2, 13);
				} else {
					c.getPA().addSkillXP(xp * SMITHING_XP, 13);
				}
	...
	public void readInput(final int level, final String type, final Player c,
		...
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				if (c.isPVPActive == true) {
					c.getPA().addSkillXP(exp * SMITHING_XP * 2,
							c.playerSmithing);
				} else {
					c.getPA().addSkillXP(exp * SMITHING_XP,
							c.playerSmithing);
				}
