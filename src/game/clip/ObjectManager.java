...
public class ObjectManager {
	...
	public static void appendStateChange(int objectType, int objectX,
			int objectY, int objectHeight, int objectFace,
			int objectChangeState, int objectVType) {
		stateChanges.add(new StateObject(objectType, objectX, objectY,
				objectFace, objectHeight, objectChangeState, objectVType));
	}

	public static void appendVarObject(int objectType, int objectX,
			int objectY, int objectHeight, int objectFace) {
		varObjects.add(new VariableObject(objectType, objectX, objectY,
				objectFace, objectHeight));
	}

	public static void removeStateChange(int objectType, int objectX,
			int objectY, int objectHeight) {
	...
	public static void removeVarObject(int objectType, int objectX,
		...
		for (int index = 0; index < varObjects.size(); index++) {
			...
			if (vo == null)
				continue;
			if (vo.getType() == objectType && vo.getX() == objectX
					&& vo.getY() == objectY && vo.getHeight() == objectHeight) {
	...
	public static boolean varObjectExists(int objectX, int objectY,
			int objectHeight) {
	...
	public static boolean stateHasChanged(int objectType, int objectX,
			int objectY, int objectHeight) {
		for (StateObject so : stateChanges) {
			if (so.getHeight() != objectHeight)
				continue;
			if (so.getX() == objectX && so.getY() == objectY
					&& so.getType() == objectType)
				return true;
		}
		return false;
	...
	private static void loadCachedObjects() {
		try {
			...
			for (int index = 0; index < length; index++)
				cachedObjects.put(
						dis.readInt(),
						new CachedObject(dis.readUnsignedShort(), dis
	...
	public static int getOrientation(int objectX, int objectY, int objectHeight) {
		...
		return (cachedObjects.get(key) != null ? cachedObjects.get(key)
				.getOrientation() : 0);
	}

	public static void changeOrientation(int objectX, int objectY,
			int objectHeight, int newO) {
		if (cachedObjects.size() == 0)
			loadCachedObjects();
		int key = (objectHeight << 30) + (objectX << 15) + (objectY);
	...
	public static boolean isCachedObject(int objectX, int objectY,
			int objectHeight, int objectId) {
		for (StateObject so : stateChanges) {
			if (so == null)
				continue;
			if (so.getHeight() != objectHeight)
				continue;
			if (so.getStatedObject() == objectId && so.getX() == objectX
					&& so.getY() == objectY)
				return true;
		}
		return false;
	...
	public static StateObject getStateObject(int objectX, int objectY,
			int objectHeight, int objectId) {
		for (StateObject so : stateChanges) {
			if (so == null)
				continue;
	...
	public static boolean objectExists(int objectX, int objectY,
			int objectHeight, int objectId) {
		if (cachedObjects.size() == 0)
			loadCachedObjects();
		int key = (objectHeight << 30) + (objectX << 15) + (objectY);
		return (cachedObjects.get(key) != null ? cachedObjects.get(key).getId() == objectId
				: false);
	}
	...
	public static void loadObjectSizes() {
		try {
			java.io.DataInputStream dis = new java.io.DataInputStream(
					new java.io.FileInputStream("./Data/object_size_config"));
			int size = (int) (new java.io.File("./Data/object_size_config")
					.length() / 2);
			for (int index = 0; index < size; index++) {
				int bitPart = dis.readShort();
				objectSizes.put(index, new int[] { ((bitPart >> 8) & 0xff),
						((bitPart) & 0xff) });
			}
			dis.close();
		} catch (Exception e) {
	...
	public static boolean withinRange(int objectType, int objectX, int objectY,
			int playerX, int playerY, int atHeight) {
		...
		java.awt.Rectangle objectField = new java.awt.Rectangle(objectX,
				objectY, sizeX, sizeY);
		java.awt.Rectangle playerField = new java.awt.Rectangle(objectX - 1,
				(objectY - 1), (sizeX + 2), (sizeY + 2));
		return playerField.contains(playerX, playerY)
				&& !objectField.contains(playerX, playerY);
	}

	public static boolean withinDoorRange(int objectType, int objectX,
			int objectY, int playerX, int playerY, int atHeight) {
		if (objectSizes.size() == 0)
			loadObjectSizes();
		boolean isOpen = isCachedObject(objectX, objectY, atHeight, objectType);
		if (isOpen) {
			StateObject so = getStateObject(objectX, objectY, atHeight,
					objectType);
			int face = so.getFace();
			if (face == 1 || face == 3)
				return playerX >= objectX - 1 && playerX <= objectX + 1
						&& playerY == objectY;
			else
				return playerY >= objectY - 1 && playerY <= objectY + 1
						&& playerX == objectX;
		} else {
			int face = getOrientation(objectX, objectY, atHeight);
			if (face == 1 || face == 3)
