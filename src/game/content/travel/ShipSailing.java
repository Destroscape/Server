...
public class ShipSailing {
	...
	private static final int[][] TRAVEL_DATA = { {}, // 0 - Null
			{ 2853, 3237, 7000 }, // 1 - From Port Sarim to Entrana
	...
	public static int getTime(final int i) {
		return ShipSailing.TRAVEL_DATA[i][2];
	}

	public static int getX(final int i) {
		return ShipSailing.TRAVEL_DATA[i][0];
	}

	public static int getY(final int i) {
		return ShipSailing.TRAVEL_DATA[i][1];
	}
	...
	public static void startTravel(final Player player, final int i) {
		...
		EventManager.getSingleton().addEvent(new Event() {
			...
			public void execute(final EventContainer e) {
				player.getPA().movePlayer(ShipSailing.getX(i),
						ShipSailing.getY(i), 0);
				e.stop();
				player.getPA().sendFrame36(75, -1);
				player.getPA().closeAllWindows();
			}
		}, ShipSailing.getTime(i) - 400);
