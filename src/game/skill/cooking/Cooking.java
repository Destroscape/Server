...
public class Cooking extends SkillHandler {
	...
	public static void cookThisFood(Player p, int i, int object) {
		...
		case 389:
			...
			break;
		case 15270:
			cookFish(p, i, 216, 99, i + 4, i + 2, object);
			break;
		case 7944:
	...
	private static void cookFish(Player c, int itemID, int xpRecieved,
		...
		int chance = (int) (c.playerLevel[7] * 1.35);
		if (c.playerEquipment[c.playerHands] == 775) {
			chance = (int) (c.playerLevel[7] * 2.5);
		}
		if (chance <= 0) {
	...
	private static void cookTheFish(final Player c) {
		...
		if (c.playerSkillProp[7][5] > 0) {
			//c.setAnimation(Animation.create(c.playerSkillProp[7][5] == 25465 ? 897 : 896));
			...
			public void execute(CycleEventContainer container) {
				...
				if (c.playerLevel[7] >= fishStopsBurning(c.playerSkillProp[7][0])
					...
					c.getItems().addItem(c.playerSkillProp[7][4], 1);
						if (c.playerEquipment[c.playerHands] == 13857) {
