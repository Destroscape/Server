		...
		for (final leatherData l : leatherData.values()) {
			if (buttonId == l.getButtonId(buttonId)) {
				if (c.leatherType == l.getLeather()) {
					...
					if (!c.getItems().playerHasItem(c.leatherType,
							l.getHideAmount())) {
						c.sendMessage("You need "
								+ l.getHideAmount()
								+ " "
								+ c.getItems().getItemName(c.leatherType)
										.toLowerCase()
								+ " to make "
					...
					//c.setAnimation(Animation.create(1249));
							...
							new CycleEvent() {
								...
								public void execute(
									...
									if (c.isSkilling[12] == true) {
										...
										c.getItems().deleteItem(1734,
												c.getItems().getItemSlot(1734),
												1);
										c.getItems().deleteItem(c.leatherType,
												l.getHideAmount());
