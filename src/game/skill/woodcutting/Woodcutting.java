...
public class Woodcutting extends SkillHandler {
				...
				if(Tree != null) {
					CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
						...
						public void execute(CycleEventContainer container) {
							if (!noInventorySpace(c, "woodcutting") || !c.stopPlayerSkill || !c.playerIsWoodcutting
							...
							if(c.playerSkillProp[8][6] > 0) {
								if (c.playerEquipment[c.playerWeapon] == SkillingTools.infernoAdze)
									c.getSkillTools().cutAndFire(c, c.playerSkillProp[8][6]);
								else
									c.getItems().addItem(c.playerSkillProp[8][6], 1);
							}
							if(c.playerSkillProp[8][3] > 0) {
							if (c.playerEquipment[c.playerHands] == 13850) {
								c.getPA().addSkillXP(c.playerSkillProp[8][3] * WOODCUTTING_XP * 2, 8);
							} else {
							...
							if(c.playerSkillProp[8][6] == 1513) {
								AchievementManager.increase(c, Achievements.LUMBERJACKING);
							}
								c.sendMessage("You get some " + c.getItems().getItemName(c.playerSkillProp[8][6]) + ".");
								AchievementManager.increase(c, Achievements.LUMBERJACK);
							}
							if(Misc.random(100) == 0) {
								recieveBirdsNest(c);
								AchievementManager.increase(c, Achievements.BIRDIE);
							}
							if (!hasAxe(c)) {
								c.sendMessage("You need a Woodcutting axe which you need a Woodcutting level to use.");
								container.stop();
							}
							if((tree != 1292 && Misc.random(100) <= Tree.getDecayChance()) && c.playerIsWoodcutting) {
					...
					CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
						...
						public void execute(CycleEventContainer container) {
							...
							if(c.playerSkillProp[8][4] > 0 && c.playerIsWoodcutting) {
								c.startAnimation(c.playerSkillProp[8][4]);
							}
						}
						@Override
	...
	public static void createStump(final Player c, final int tree, final int obX, final int obY) {
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			@Override
			public void stop() {
				Server.objectHandler.placeObject(new Objects(tree, obX, obY, c.heightLevel, 0, 10));
				//resetWoodcutting(c);
			}
		}, Tree.getRespawnTime());
	}
	...
	private static int[][] axes = {
		...
		{1359, 41, 867, 4},
		{6739, 61, 2846, 3},
		{SkillingTools.infernoAdze, 61, 10251, 2},
	};	
