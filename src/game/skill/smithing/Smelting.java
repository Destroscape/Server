...
public class Smelting extends SkillHandler {
	...
	public static int[][] data = {
		...
		{ 3, 20, 17, IRON, -1, -1, 2353 }, // Steel
		{ 4, 50, 30, MITH, COAL, 1, 2359 }, // Mith
		{ 5, 70, 37, ADDY, COAL, 1, 2361 }, // Addy
		{ 6, 85, 50, RUNE, COAL, 1, 2363 }, // Rune
		/** ---------------OTHERS------------/ */
			...
			if (bartype == data[i][0]) {
				...
				if (data[i][3] > 0 && data[i][4] > 0) { // All OTHER bars
					if (!c.getItems().playerHasItem(data[i][3])
							|| !c.getItems().playerHasItem(data[i][4])) {
						c.sendMessage("You need an "
								...
								+ data[i][5]
										+ " "
										+ c.getItems().getItemName(data[i][4])
				...
				if (data[i][5] > 0) { // Bars with more than 1 coal requirement
					if (!c.getItems().playerHasItem(data[i][4], data[i][5])) {
						c.sendMessage("You need an "
								+ c.getItems().getItemName(data[i][3])
								.toLowerCase() + " and " + data[i][5]
