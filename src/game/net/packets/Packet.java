...
public final class Packet {
	...
	private static String byteToHex(final byte b, final boolean forceLeadingZero) {
		final StringBuilder out = new StringBuilder();
		final int ub = b & 0xff;
		if (ub / 16 > 0 || forceLeadingZero) {
			out.append(Packet.hex[ub / 16]);
		}
		out.append(Packet.hex[ub % 16]);
		return out.toString();
	}
	...
	public Packet(final IoSession session, final int pID, final byte[] pData,
			final boolean bare, final Size s) {
	...
	public void readBytes(final byte[] buf, final int off, final int len) {
		for (int i = 0; i < len; i++) {
	...
	public String toString() {
		...
		for (int x = 0; x < pLength; x++) {
			sb.append(Packet.byteToHex(pData[x], true));
		}
		sb.append("]");
		return sb.toString();
	}
