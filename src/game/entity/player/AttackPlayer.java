...
public class AttackPlayer implements PacketType {
	...
	public void processPacket(final Player c, final int packetType,
		...
		case ATTACK_PLAYER:
			...
			/*
			 * if (PlayerHandler.players[c.playerId].connectedFrom != null) {
			 * c.sendMessage("You cannot attack yourself."); return; }
			 */
			if (c.respawnTimer > 0) {
				break;
			}
			...
			if (c.duelStatus == 5) {
				...
				if (c.duelRule[9]) {
					...
					for (final int funWeapon : Config.FUN_WEAPONS) {
			...
			if (c.playerEquipment[c.playerWeapon] == 15241
				...
				c.sendMessage("You need hand cannot shots to fire this weapon!");
				c.stopMovement();
				c.getCombat().resetPlayerAttack();
				return;
			}
