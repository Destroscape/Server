...
public class Lunar {
	...
	public static void magicOnItems(final Player c, int slot, int itemId,
			int spellId) {
		...
		case 30017:
			...
			if (!c.getItems().playerHasItem(astral)
					|| !c.getItems().playerHasItem(fire, 5)
					|| !c.getItems().playerHasItem(water, 4)) {
			...
			if (itemId == 2317 || itemId == 2319 || itemId == 2321) {
				...
				c.getItems().deleteItem(astral,
						c.getItems().getItemSlot(astral), 1);
				c.getItems()
						.deleteItem(fire, c.getItems().getItemSlot(fire), 5);
				c.getItems().deleteItem(water, c.getItems().getItemSlot(water),
						4);
		...
		case 30154:
			...
			if (!c.getItems().playerHasItem(astral, 2)
					|| !c.getItems().playerHasItem(fire, 6)
					|| !c.getItems().playerHasItem(air, 10)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			if (itemId != 1783) {
			...
			c.getItems()
					.deleteItem(astral, c.getItems().getItemSlot(astral), 2);
	...
	public static void Button(final Player c, int actionButtonId) {
		if (System.currentTimeMillis() - c.lunarDelay < 2000)
			return;
		...
		case 117104:
			...
			if (!c.getItems().playerHasItem(astral)
					|| !c.getItems().playerHasItem(water, 3)
					|| !c.getItems().playerHasItem(fire)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			if (c.getItems().playerHasItem(229)) {
				...
				c.getItems().deleteItem(astral,
						c.getItems().getItemSlot(astral), 1);
				c.getItems()
						.deleteItem(fire, c.getItems().getItemSlot(fire), 1);
				c.getItems().deleteItem(water, c.getItems().getItemSlot(water),
						3);
		...
		case 117147:
			...
			if (!c.getItems().playerHasItem(astral, 2)
					|| !c.getItems().playerHasItem(earth, 2)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			...
			c.gfx0(1074);
			c.getItems()
					.deleteItem(astral, c.getItems().getItemSlot(astral), 1);
		...
		case 117170:
			...
			if (!c.getItems().playerHasItem(astral, 2)
					|| !c.getItems().playerHasItem(cosmic, 2)
					|| !c.getItems().playerHasItem(law, 2)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			c.getItems()
					.deleteItem(astral, c.getItems().getItemSlot(astral), 2);
			c.getItems()
					.deleteItem(cosmic, c.getItems().getItemSlot(cosmic), 2);
		...
		case 118106:
			...
			if (!c.getItems().playerHasItem(astral, 4)
					|| !c.getItems().playerHasItem(blood, 3)
					|| !c.getItems().playerHasItem(law, 6)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			c.getItems()
					.deleteItem(astral, c.getItems().getItemSlot(astral), 4);
		...
		case 118034:
			...
			if (!c.getItems().playerHasItem(astral, 3)
					|| !c.getItems().playerHasItem(earth, 15)
					|| !c.getItems().playerHasItem(nats, 1)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			...
			c.getItems()
					.deleteItem(astral, c.getItems().getItemSlot(astral), 3);
		...
		case 117226:
			...
			if (!c.getItems().playerHasItem(astral, 2)
					|| !c.getItems().playerHasItem(cosmic)
					|| !c.getItems().playerHasItem(body, 5)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			if (c.indream) {
			...
			c.getItems()
					.deleteItem(astral, c.getItems().getItemSlot(astral), 2);
			c.getItems()
					.deleteItem(cosmic, c.getItems().getItemSlot(cosmic), 1);
	...
	public static void CastingLunarOnPlayer(final Player c, int castingSpellId) {
		final Player castOnPlayer = PlayerHandler.players[c.playerIndex];
		c.stopMovement();
		...
		case 30298:
			...
			c.getItems()
					.deleteItem(earth, c.getItems().getItemSlot(astral), 10);
		...
		case 30048:
			...
			if (!c.getItems().playerHasItem(earth, 10)
					|| !c.getItems().playerHasItem(astral)
					|| !c.getItems().playerHasItem(law)) {
				c.sendMessage("You do not have the required runes to cast this spell.");
				return;
			}
			...
			c.sendMessage("You have been cured by "
	...
	private static void energyTransfer(final Player c) {
		if (c.playerIndex > 0) {
			...
			if (!c.getItems().playerHasItem(9075, 3)
					|| !c.getItems().playerHasItem(563, 2)
			...
			if (c.specAmount == 10) {
				...
				c.getItems().deleteItem2(563, 2);// For these you need to change
													// to deleteItem(item,
													// itemslot, amount);.
	...
	private static void healOther(final Player c, Player q) {
		if (c.playerIndex > 0) {
			final int oX = q.getX();
			final int oY = q.getY();
			...
			if (!c.getItems().playerHasItem(9075, 3)
					|| !c.getItems().playerHasItem(563, 3)
					|| !c.getItems().playerHasItem(565, 1)) {
				c.sendMessage("You don't have the required runes to cast this spell.");
				c.getCombat().resetPlayerAttack();
				c.stopMovement();
				c.turnPlayerTo(oX, oY);
			...
			if (q.playerLevel[3] + q.playerLevel[3] * .75 > q.playerLevel[3]) {
				...
				c.getItems()
						.deleteItem(9075, c.getItems().getItemSlot(9075), 3);
	...
	private static void cureAll(Player c) {
	...
	private static void HealAll(Player c) {
		for (Player p : PlayerHandler.players) {
			...
			if (p != null && person.distanceToPoint(c.absX, c.absY) <= 1) {
				...
				if (castOn.playerLevel[3] + (2 * castOn.playerLevel[3]) > castOn.playerLevel[3])
					castOn.playerLevel[3] = castOn.getPA().getLevelForXP(
							c.playerXP[3]);
