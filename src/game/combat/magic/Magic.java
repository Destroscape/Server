...
public class Magic {
	...
	public static enum Enchant {
		...
		RUBYRING(1641, 2568, 47, 59, 720, 115, 3), RUBYAMULET(1698, 1725, 47,
				59, 720, 115, 3), RUBYNECKLACE(1660, 11194, 47, 59, 720, 115, 3),

		DIAMONDRING(1643, 2570, 57, 67, 720, 115, 4), DIAMONDAMULET(1700, 2570,
				57, 67, 720, 115, 4), DIAMONDNECKLACE(1662, 11090, 57, 67, 720,
				115, 4),

		DRAGONSTONERING(1645, 2572, 68, 78, 721, 116, 5), DRAGONSTONEAMULET(
				1702, 1712, 68, 78, 721, 116, 5), DRAGONSTONENECKLACE(1664,
				11105, 68, 78, 721, 116, 5),

		ONYXRING(6575, 6583, 87, 97, 721, 452, 6), ONYXAMULET(6581, 6585, 87,
				97, 721, 452, 6), ONYXNECKLACE(6577, 11128, 87, 97, 721, 452, 6);
		...
		private static Enchant getID(final int ID) {
			for (Enchant e : values()) {
				if (e.getUnenchanted() == ID) {
					return e;
				}
			}
			return null;
	...
	private static enum EnchantSpell {
		...
		private static EnchantSpell getID(final int ID) {
			for (EnchantSpell es : values()) {
				if (es.getSpell() == ID) {
					return es;
				}
			}
	...
	private boolean hasRunes(int spellID) {
		EnchantSpell ens = EnchantSpell.getID(spellID);
		if (ens == null) {
			return false;
		}
		if (ens.getReq3() == 0) {
			return c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1())
					&& c.getItems().playerHasItem(ens.getReq2(),
							ens.getReqAmt2())
					&& c.getItems().playerHasItem(ens.getReq3(),
							ens.getReqAmt3());
		} else {
			return c.getItems().playerHasItem(ens.getReq1(), ens.getReqAmt1())
					&& c.getItems().playerHasItem(ens.getReq2(),
	...
	public void enchantItem(int itemID, int spellID) {
		Enchant enc = Enchant.getID(itemID);
		EnchantSpell ens = EnchantSpell.getID(spellID);
			...
			if (c.getItems().playerHasItem(enc.getUnenchanted(), 1)) {
				if (hasRunes(spellID)) {
					if (getEnchantmentLevel(spellID) == enc.getELevel()) {
						...
						c.getItems().deleteItem(ens.getReq1(),
								c.getItems().getItemSlot(ens.getReq1()),
								ens.getReqAmt1());
						c.getItems().deleteItem(ens.getReq2(),
								c.getItems().getItemSlot(ens.getReq2()),
								ens.getReqAmt2());
						//c.setAnimation(Animation.create(enc.getAnim()));
						c.startAnimation(enc.getAnim());
						c.gfx100(enc.getGFX());
						if (ens.getReq3() != -1) {
							c.getItems().deleteItem(ens.getReq3(),
									c.getItems().getItemSlot(ens.getReq3()),
									ens.getReqAmt3());
						}
						c.getPA().sendFrame106(6);
					} else {
						c.sendMessage("You can only enchant this jewelry using a level-"
