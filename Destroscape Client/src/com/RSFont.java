...
public class RSFont extends DrawingArea {
	...
	public RSFont(boolean TypeFont, String s, StreamLoader archive) {
		try {
			int length = (s.equals("regularhit") || s.equals("bighit")) ? 58
					: 256;
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (character == 60) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							...
							try {
								...
								int iconModY = icon.myHeight + icon.anInt1443
										+ 1;
								if (transparency == 256) {
									icon.drawSprite(drawX, (drawY
											+ baseCharacterHeight - iconModY));
				...
				if (startIndex == -1) {
					...
					if (underlineColor != -1) {
						DrawingArea.drawHorizontalLine(drawX, drawY
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								modifierOffset++;
								int iconId = Integer.valueOf(effectString
										.substring(7));
		...
		do {
			try {
				if (string.startsWith(startColor)) {
					...
					textColor = color.length() < 6 ? Color.decode(color)
							.getRGB() : Integer.parseInt(color, 16);
					...
					String color = string.substring(4);
					strikethroughColor = color.length() < 6 ? Color.decode(
							color).getRGB() : Integer.parseInt(color, 16);
					...
					underlineColor = color.length() < 6 ? Color.decode(color)
							.getRGB() : Integer.parseInt(color, 16);
					...
					textShadowColor = color.length() < 6 ? Color.decode(color)
							.getRGB() : Integer.parseInt(color, 16);
					...
					setDefaultTextEffectValues(defaultColor, defaultShadow,
							defaultTransparency);
				}
			} catch (Exception exception) {
				break;
	...
	public static String handleOldSyntax(String text) {
		...
		text = text.replaceAll("@don1@", "<img=4>");
		text = text.replaceAll("@don2@", "<img=5>");
		text = text.replaceAll("@info@", "<img=6>");
		text = text.replaceAll("@img@", "<img=7>");
		text = text.replaceAll("@img1@", "<img=8>");
		return text;
	}
	...
	public void drawTransparentCharacter(int i, int i_11_, int i_12_,
		...
		if (i_13_ > 0 && i_14_ > 0) {
			createTransparentCharacterPixels(DrawingArea.pixels, fontPixels[i],
	...
	public void drawCharacter(int character, int i_35_, int i_36_, int i_37_,
		...
		if (i_37_ > 0 && i_38_ > 0) {
			try {
				createCharacterPixels(DrawingArea.pixels,
