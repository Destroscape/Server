...
public final class TextDrawingArea extends DrawingArea {
	...
	public TextDrawingArea(boolean flag, String s, StreamLoader streamLoader) {
		try {
			int length = (s.equals("hit_full") || s.equals("critical_full")) ? 58
					: 256;
			...
			Stream stream_1 = new Stream(
					streamLoader.getDataForName("index.dat"));
	...
	public void method386(int i, String s, int j, int k, int l) {
		...
		for (int i1 = 0; i1 < s.length(); i1++) {
			char c = s.charAt(i1);
			if (c != ' ')
				method392(aByteArrayArray1491[c], j + anIntArray1494[c], l
						+ anIntArray1495[c]
						+ (int) (Math.sin(i1 / 2D + k / 5D) * 5D),
		...
		for (int i1 = 0; i1 < s.length(); i1++) {
			...
			if (c != ' ')
				method392(aByteArrayArray1491[c], i + anIntArray1494[c]
						+ (int) (Math.sin(i1 / 5D + j / 5D) * 5D), k
						+ anIntArray1495[c]
						+ (int) (Math.sin(i1 / 3D + j / 5D) * 5D),
	...
	public void method388(int i, String s, int j, int k, int l, int i1) {
		...
		for (int k1 = 0; k1 < s.length(); k1++) {
			char c = s.charAt(k1);
			if (c != ' ')
				method392(aByteArrayArray1491[c], l + anIntArray1494[c], k
						+ anIntArray1495[c]
						+ (int) (Math.sin(k1 / 1.5D + j) * d),
	...
	public void method389(boolean flag1, int i, int j, String s, int k) {
		...
		if (aBoolean1499)
			DrawingArea.method339(k + (int) (anInt1497 * 0.69999999999999996D),
					0x800000, i - l, l);
	}
