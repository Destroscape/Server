...
public final class Sprite extends DrawingArea {
	...
	public Sprite(String img, int width, int height, int nothing) {
		try {
	...
	public void draw24BitSprite(int x, int y) {
		...
		if (!((width <= 0) || (height <= 0))) {
			set24BitPixels(width, height, DrawingArea.pixels, myPixels, alpha,
					destOffset, srcOffset, destStep, srcStep);
		}
	}
	...
	private void set24BitPixels(int width, int height, int destPixels[],
			int srcPixels[], int srcAlpha, int destOffset, int srcOffset,
		...
		for (int loop = -height; loop < 0; loop++) {
			for (int loop2 = -width; loop2 < 0; loop2++) {
				...
				if (srcColor != 0 && srcColor != 0xffffff) {
					...
					destPixels[destOffset++] = ((srcColor & 0xff00ff)
							* srcAlpha + (destColor & 0xff00ff) * destAlpha & 0xff00ff00)
							+ ((srcColor & 0xff00) * srcAlpha
	...
	public Sprite(byte abyte0[], Component component) {
		try {
			Image image = Toolkit.getDefaultToolkit().getImage(
					location + "GameFrame/Background/0.png");
	...
	private void setPixels(int width, int height, int destPixels[],
			int srcPixels[], int srcAlpha, int destOffset, int srcOffset,
			int destStep, int srcStep) {
		int srcColor;
		int destAlpha;
	...
	public void drawSprite3(int x, int y, int opacity) {
		...
		if (!((width <= 0) || (height <= 0))) {
			setPixels(width, height, DrawingArea.pixels, myPixels, alpha,
					destOffset, srcOffset, destStep, srcStep);
		}
	}
	...
	public void drawARGBSprite(int xPos, int yPos, int alpha) {
		...
		if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
			renderARGBPixels(spriteWidth, spriteHeight, myPixels,
					DrawingArea.pixels, i1, alphaValue, j1, j2, i2);
		}
	}

	private void renderARGBPixels(int spriteWidth, int spriteHeight,
			int spritePixels[], int renderAreaPixels[], int pixel,
			int alphaValue, int i, int l, int j1) {
		int pixelLevel;
		int alphaLevel;
		for (int height = -spriteHeight; height < 0; height++) {
			for (int width = -spriteWidth; width < 0; width++) {
				...
				if (pixelLevel != 0) {
					...
					renderAreaPixels[pixel++] = ((pixelLevel & 0xff00ff)
							* alphaValue + (pixelValue & 0xff00ff) * alphaLevel & 0xff00ff00)
							+ ((pixelLevel & 0xff00) * alphaValue
									+ (pixelValue & 0xff00) * alphaLevel & 0xff0000) >> 8;
				} else {
					pixel++;
				}
			}
